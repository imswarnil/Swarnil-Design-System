...
pt = ('<p class="u-fg-subtle u-mb-6" style="max-width:var(--measure-lead)">'
      ...
      + '<h2 class="t-h3" style="margin:var(--space-10) 0 var(--space-2)">The flavours</h2>'
      + '<p class="u-fg-subtle u-mb-6" style="max-width:var(--measure-lead)">Every keyframe below '
        'ships in <code class="t-code">05-motion.css</code>. The pages that used to demonstrate them '
        'left with <code class="t-code">collection/</code> when the theme was split into its own '
        'repo, so these are the definitions rather than a live click-through — a destination in this '
        'system sets <code class="t-code">data-transition</code> on its own shell and gets the '
