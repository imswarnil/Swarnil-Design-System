...
Re-run after editing content or fragments:
    npm run docs        (= python3 docs/_build/build.py)
...
GROUP_ICONS = {
 ...
 'Patterns': '<rect x="3.5" y="3.5" width="7" height="7" rx="1.5"/><rect x="13.5" y="3.5" width="7" height="7" rx="1.5"/><rect x="3.5" y="13.5" width="7" height="7" rx="1.5"/><path d="M13.5 17h7M17 13.5v7"/>',
...
NAV = [
    ...
    # Layer 4 · patterns — recurring ARRANGEMENTS of components, which is what
    # makes a site rather than a kit. These four came out of the old
    # 'Collections' group when collection/ was split off to the theme repo: the
    # group documented a Ghost/Jekyll theme, but these four describe the system
    # itself and stayed. approach.md §9 names the rest of this layer (.deck,
    # .index, .prose, .chat, .compare) — added in Phase 6.
    ('Patterns', [('timeline', 'Timeline'),
                  ('travel/itinerary', 'Itinerary'),
                  ('course/curriculum', 'Curriculum'),
                  ('projects/build-log', 'Build log')]),
...
def render(slug, title, group, lead, body, opts, return_toc=False):
    ...
    out = TEMPLATE.format(
        ...
        # collection/ moved to the theme repo — the design system no longer
        # ships theme stylesheets. Kept as an empty slot so the page template
        # does not need editing; the placeholder goes when Phase 5 replaces
        # this generator.
        collection_css='')
...
REDIRECTS = {
    ...
    # The collection docs left with collection/ when it was split to the theme
    # repo. No redirect target exists here any more, so those slugs are gone
