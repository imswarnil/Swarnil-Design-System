...
// Fixture URLs point at the DOCS page that demonstrates that kind of thing —
// a post at the post page, a tag at the tag page, the author at About. In a
// theme these are Ghost's own urls; here, a reader clicking a demo card
// should land somewhere real rather than on a 404.
export function loadFixtures() {
	...
	const posts = raw.map((p, i) => {
		const tags = p.tags.map((t) => {
			if (!tagMap.has(t.slug)) tagMap.set(t.slug, { ...t, id: `t-${t.slug}`, url: '/collections/tag/page/', visibility: 'public', count: { posts: 0 } });
